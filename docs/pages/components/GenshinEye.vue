...
onMounted(() => {
  ...
  // 加载纹理图片
  const bg = textureloader.load('/img/77a3b856-6651-4956-85fa-8b05ad374dc5.jpg')
  // 将纹理叠加到材质上
  ...
  // 创建矩形模型
  const pendantBox = new THREE.PlaneGeometry(15, 11)
  // 创建基础材质
  const pendantMaterial = new THREE.MeshBasicMaterial()
  // 创建纹理对象
  const pendanttexture = new THREE.TextureLoader()
  // 加载纹理图片
  const pendanttextureload = pendanttexture.load('/img/37b8e838-7b22-4f8d-b100-fe1464843236.png')
  // 将纹理叠加到材质上
  ...
  // 将材质叠加到模型上
  const pendantMesh = new THREE.Mesh(pendantBox, pendantMaterial)
  // 移动模型
  ...
  // 创建矩形模型
  const pendantLightBox = new THREE.PlaneGeometry(15, 11)
  // 创建基础材质
  const pendantLightMaterial = new THREE.MeshBasicMaterial()
  // 创建纹理对象
  const pendantLighttexture = new THREE.TextureLoader()
  // 加载纹理图片
  const pendantLighttextureload = pendantLighttexture.load('/img/1fe2462e-7aaa-4c13-ad10-7ff6fa973d31.png')
  // 将纹理叠加到材质上
  ...
  // 将材质叠加到模型上
  const pendantLightMesh = new THREE.Mesh(pendantLightBox, pendantLightMaterial)
  ...
  // 创建矩形模型
  const arrowsBox = new THREE.PlaneGeometry(3, 3)
  // 创建基础材质
  const arrowsMaterial = new THREE.MeshBasicMaterial()
  // 创建纹理对象
  const arrowsTexture = new THREE.TextureLoader()
  // 加载纹理图片
  const arrowsTextureload = arrowsTexture.load('/img/8527e88b-685a-46cd-aec8-4ab73403dce3.png')
  // 将纹理叠加到材质上
  arrowsMaterial.map = arrowsTextureload
  // png图片透明部分开启
  arrowsMaterial.transparent = true
  // 将材质叠加到模型上
  const arrowsMesh = new THREE.Mesh(arrowsBox, arrowsMaterial)
  // 移动模型
  ...
  const light = textureloader.load('/img/b9047335-f041-4c4b-a7c3-449fd5d5ff7c.jpg')
  lightplaneMaterial.map = light
  lightplaneMaterial.blending = THREE.AdditiveBlending
  lightplaneMaterial.transparent = true
  const lightMesh = new THREE.Mesh(lightplane, lightplaneMaterial)
  ...
  // 创建关键帧
  const inScaleTrack = new THREE.KeyframeTrack('pendantLight.scale', [0, 30], [0.5, 0.5, 0.5, 1, 1, 1])
  const inOpacityTrack = new THREE.KeyframeTrack('pendantLight.material.opacity', [0, 30], [0, 1])
  // 动画持续时间（帧）
  const duration = 60
  // 创建剪辑对象
  const clip = new THREE.AnimationClip('in', duration, [inScaleTrack, inOpacityTrack])
  // 创建混合器对象
  ...
  function addDot(z, sc, i = 1) {
    ...
    const randomY = Math.random() * 50 - 25 * i
    const dotPlane = new THREE.PlaneGeometry(1, 1)
    const dotPlaneMaterial = new THREE.MeshBasicMaterial()
    const dotTexture = new THREE.TextureLoader()
    const dotTextureload = dotTexture.load('/img/d0d14aff-ecee-4449-806c-4a866163a7ce.jpg')
    dotPlaneMaterial.map = dotTextureload
    dotPlaneMaterial.blending = THREE.AdditiveBlending
    dotPlaneMaterial.transparent = true
    const dotMesh = new THREE.Mesh(dotPlane, dotPlaneMaterial)
  ...
  const bgMusic = document.querySelector('#bg-music audio')
  // 把静音图标隐藏
  ...
  function mouseEvent(e) {
    ...
    // 返回射线选中的对象
    const intersects = raycaster.intersectObjects([pendantMesh])
    if (intersects.length > 0) {
      ...
      // 获取鼠标移出物体时在第几帧
      const timing = AnimationAction.time
      // 从第几帧开始播放
  ...
  const render = function () {
    ...
    for (let i = 0; i < group1.children.length; i++) {
      ...
      if (item.material.opacity <= 0) {
        const itemArguments = [item.position.z, item.scale.x]
        group1.remove(item)
        moveArrX.splice(i, 1)
        moveArrY.splice(i, 1)
        moveTime.splice(i, 1)
...
<template>
  <div ref="container" class="w-1920px h-1080px transform-origin-lt">
    <div id="bg-music">
      <audio src="/music/desk-bgm.e1c8e207.mp3" loop autoplay />
    </div>
    <div id="music-button">
